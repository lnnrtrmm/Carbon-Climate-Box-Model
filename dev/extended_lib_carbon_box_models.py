...
class EnergyAndCarbonBoxModel:
    ...
    def spinup(self, co2fix=283.0, years=3000, silent=False):
        if not self.ccycle_type == 'CarbonModel': 
            print('The chosen Carbon Model ("'+self.ccycle_type+'") does not allow a spinup!')
    ...
    def integrate(self, silent=False):
        ...
        if not silent: print('Start integrating', self.ccycle_type, '...')
    ...
    class CarbonBoxModel:
        ...
        T0_warm_thermo        = 12.2
        T_warm_thermo_TTC_dep = 1.02
    
        T0_cold_deep       = 1.61
        T_cold_deep_TD_dep = 1.59
        ...
        S0_warm_thermo = 34.856 # assumed to be constant
        S0_cold_deep   = 34.681 # assumed to be constant
    
        # pot. density
        rho0_warm_surface = calc_potential_density(T0_warm_surface, S0_warm_surface)
        rho0_cold_surface = calc_potential_density(T0_cold_surface, S0_cold_surface)
        rho0_warm_thermo  = calc_potential_density(T0_warm_thermo, S0_warm_thermo)
        rho0_cold_deep    = calc_potential_density(T0_cold_deep, S0_cold_deep)

        rho0_frac_wi = (rho0_warm_thermo - rho0_warm_surface) / rho0_warm_surface
        rho0_frac_cd = (rho0_cold_deep - rho0_cold_surface) / rho0_cold_surface
        ...
        def __init__(self, co2_emissions, sy=1850, ey=2300, dt=1.0, dbg=0, lfix_co2=False, co2fix=283.0,
            ...
            # Mixing rates dependent on stratification?
            # self.include_variable_mixing = False
            ...
            ## Include approximation for sea ice?
            self.include_ice = False
            ...
            self.ice_area = 0.0 # overwritten if include_ice = True
            self.k_gasex_cold = self.k_gasex # overwritten if include_ice = True
            ...
            self.k_T = np.zeros((self.nyears))
            self.k_wi = np.zeros((self.nyears))
            self.k_cd = np.zeros((self.nyears))
        ...
        def step_forward(self, i, T_surf, T_thermo, T_deep):
            ...
            if self.advection=='Lenton':
                ...
                tmp_C_warm, tmp_C_cold, tmp_C_int, tmp_C_deep = self.__advect_ocean_tracer_LOSCAR(i,
                                                                                T_surf, T_thermo, T_deep,
                                                                                self.C_warm_surf[i] / self.V_warm,
                                                                                self.C_cold_surf[i] / self.V_cold,
                                                                                self.C_int_wat[i]   / self.V_int,
                                                                                self.C_deep_oce[i]  / self.V_deep)
        ...
        def __calc_surface_ocean_flux(self, i, Ts):
            ...
            if self.include_ice:
                self.ice_area = -2.12e12 * Ts + 16.3e12
                self.k_gasex_cold = self.k_gasex * self.ocean_surface_area * self.cold_area_fraction \
                                               / (self.ocean_surface_area * self.cold_area_fraction - 16.3e12)
            ...
            self.ocean_flux_cold[i] = self.k_gasex_cold * self.mol_to_GT_C \
                                     * (self.ocean_surface_area * self.cold_area_fraction - self.ice_area) \
        ...
        def __advect_ocean_tracer_LOSCAR(self, i, Ts, T_TC, T_D, C_w, C_c, C_i, C_d):
            ### assumes a list of tracer concentrations is given in order:
            ## C_warm, C_cold, C_int, C_deep
        
            self.__calculate_transport_rates_LOSCAR(i, Ts, T_TC, T_D)
            
            Cw = C_w + (self.dts / self.V_warm) * ( self.k_wi[i] * (C_i - C_w) )
            Cc = C_c + (self.dts / self.V_cold) * ( self.k_T[i] * (C_i - C_c) + self.k_cd[i] * (C_d - C_c) )
            Ci = C_i + (self.dts / self.V_int)  * ( self.k_T[i] * (C_d - C_i) - self.k_wi[i] * (C_i - C_w) )
            Cd = C_d + (self.dts / self.V_deep) * ( self.k_T[i] * (C_c - C_d) - self.k_cd[i] * (C_d - C_c) )
        
            return Cw, Cc, Ci, Cd
    
    
        def __calculate_transport_rates_LOSCAR(self, i, Ts, T_TC, T_D):
            
            # Conveyor Transport
            # ToDo: Check wether this should also be made dependent on stratification
            self.k_T[i] = self.k_conveyor        
            if self.include_Tdep_advection: self.k_T[i]  += self.advection_Tdep_frac * self.k_conveyor * Ts
            
            # Mixing coefficients
            if self.include_variable_mixing: 
                
                T_warm_surf   = self.T0_warm_surface + self.T_warm_surface_Tsdep  * Ts
                T_cold_surf   = self.T0_cold_surface + self.T_cold_surface_Tsdep  * Ts
                T_warm_thermo = self.T0_warm_thermo  + self.T_warm_thermo_TTC_dep * T_TC
                T_cold_deep   = self.T0_cold_deep    + self.T_cold_deep_TD_dep    * T_D
                
                S_warm_surf   = self.S0_warm_surface + self.S_warm_surface_Tsdep * Ts
                S_cold_surf   = self.S0_cold_surface + self.S_cold_surface_Tsdep * Ts
                S_warm_thermo = self.S0_warm_thermo
                S_cold_deep   = self.S0_cold_deep
            
                
            
                rho_warm_surf   = calc_potential_density(T_warm_surf, S_warm_surf)
                rho_cold_surf   = calc_potential_density(T_cold_surf, S_cold_surf)
                rho_warm_thermo = calc_potential_density(T_warm_thermo, S_warm_thermo)
                rho_cold_deep   = calc_potential_density(T_cold_deep, S_cold_deep)
            
                self.k_wi[i] = self.k_mix_warm_int * self.rho0_frac_wi * rho_warm_surf / (rho_warm_thermo - rho_warm_surf)
                #self.k_wi[i] = self.k_mix_warm_int
    
                self.k_cd[i] = self.k_mix_cold_deep * self.rho0_frac_cd * rho_cold_surf / (rho_cold_deep - rho_cold_surf)
                #self.k_cd[i] = self.k_mix_cold_deep
            
                if self.dbg==3 and i < 5:
                    print('   dbg ',i,', T_warm_surf :', T_warm_surf)
                    print('   dbg ',i,', T_cold_surf :', T_cold_surf)
                    print('   dbg ',i,', T_warm_thermo :', T_warm_thermo)
                    print('   dbg ',i,', T_cold_deep :', T_cold_deep)
                    print()
                    print('   dbg ',i,', S_warm_surf :',   S_warm_surf)
                    print('   dbg ',i,', S_cold_surf :',   S_cold_surf)
                    print('   dbg ',i,', S_warm_thermo :', S_warm_thermo)
                    print('   dbg ',i,', S_cold_deep :',   S_cold_deep)
                    print()
                    print('   dbg ',i,', rho_warm_surf :',   rho_warm_surf)
                    print('   dbg ',i,', rho_cold_surf :',   rho_cold_surf)
                    print('   dbg ',i,', rho_warm_thermo :', rho_warm_thermo)
                    print('   dbg ',i,', rho_cold_deep :',   rho_cold_deep)
                    print()
                    print('   dbg ',i,', k_wi :', self.k_wi[i]*1e-6, ' Sv')
                    print('   dbg ',i,', k_cd :', self.k_cd[i]*1e-6, ' Sv')
                    print('   dbg ',i,', k_T :',  self.k_T[i]*1e-6, ' Sv')
                    print()
                    print()
                    print()
